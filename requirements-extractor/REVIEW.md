...
### 3.16 Smart boilerplate auto-skip — LOW — ✅ FIXED (2026-04-24, late session)

Defense / aerospace / telecoms specs all repeat the same handful of boilerplate sections — Glossary, Acronyms, References, Revision History, Document Control, Table of Contents, Approvals, Distribution, etc. — that don't carry binding requirements but DO trigger the keyword classifier (e.g. a Glossary entry "*token shall mean an opaque string*" matches HARD).  Pre-skipping these by default saves users from rediscovering the same skip list on every project.

**Resolution.** New module-level `DEFAULT_BOILERPLATE_TITLES` list in `config.py` (~25 entries covering the canonical names) and a `SkipSections.auto_boilerplate: bool = True` toggle.  When the toggle is on (the default), `SkipSections.matches_title()` OR-s the default list into its existing user-`titles` substring/equality match — so both the section-row title path (`parser.py` table-walker) and the heading-scope path (new) honour it without code duplication.

The heading-scope plumbing is the meaningful new code: a `skip_heading_level: Optional[int]` field on `_ParseContext` is set when a top-level Heading paragraph matches the filter, and cleared on the next heading at the same level or shallower whose text DOESN'T match.  `_emit_candidate` short-circuits to `None` while the flag is active, so requirements emitted from any nested table or paragraph inside the skipped section drop cleanly without polluting the events stream.  Adjacent boilerplate headings (Glossary → Acronyms → References) chain correctly because the clear-then-set order means a non-matching heading is required to fully exit the skip.

The user `titles:` list still works as before and is layered on top, so house-style boilerplate ("Internal Notes", "Programme History") can be added per project without losing the defaults.  Setting `auto_boilerplate: false` in a config disables only the defaults — user titles stay active — for the rare corpus where one of the default names is a substantive section.  `samples/sample_config.yaml` documents the new key inline; the top-level Config section in the README also calls it out.

Covered by 15 tests in `test_boilerplate_skip.py`: pure-function `matches_title` (case-insensitive, numeric prefix, empty, on/off interaction with user titles), schema validator acceptance of `auto_boilerplate`, and four end-to-end docx fixtures (Glossary after real section drops only the glossary, Glossary first → real section after still extracts the real one, opt-out via `auto_boilerplate=false`, section-row path also honours boilerplate).
