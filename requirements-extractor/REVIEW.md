...
- ✅ FIXED: §1.7 (NER canonicalisation in `actors.canonicalise_ner_name`, 20 tests in `test_ner_canonicalisation.py`), §1.9 (`detector.compute_confidence` with vague/measurable signal offsets, 16 tests in `test_detector.py`), §1.10 (`models.annotate_cross_source_duplicates` wired into the extractor, 7 tests in `test_stable_ids.py`), §3.1 (`.doc` via LibreOffice shell-out **plus** `.pdf` via pdfplumber — both live in `legacy_formats.py`, 12 tests in `test_legacy_formats.py`, routed through `prepare_for_parser`), §3.10 full (JSON + Markdown in `writers_extra.py`, ReqIF 1.2 with `basic`/`cameo`/`doors` dialects in `reqif_writer.py`; CLI `--emit json,md,reqif` + `--reqif-dialect=…`; 26 tests across `test_writers_extra.py` and `test_reqif_writer.py`), §3.12 (`diff.py` + `diff` subcommand with sheet-by-name lookup and pre-stable-id rejection, 15 tests in `test_diff.py`), §3.15 (README "A note on tracked changes" section).
- Still open: §3.8 (source-preview column — approach still TBD between hyperlink-only vs inline context-snippet; needs a product decision).
